...
const ServiceFlipCard = ({ service }) => {
  const { title, image, cost, days, origin } = service;
  ...
  return (
    <div className="col-md-4 mb-5" style={{ background: "none" }}>
      <div className=" shadow border p-5">
        <div className=""></div>
        <img className="w-100" src={image} alt="Brohm Lake" /> <br />
        <br />
        <h3>{title}</h3>
        <h5>
          Imported From: <span style={{ color: "tomato" }}> {origin}</span>
        </h5>
        <div
        ...
        >
          ...
          <div class="card-footer bg-transparent border-top-0 d-flex justify-content-start">
            <h5 id="price">
              <span className="fw-bolder">৳</span>
              {cost} <small>/Pieces</small>
            </h5>
          </div>
        </div>
